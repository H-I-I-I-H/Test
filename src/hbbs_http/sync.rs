...
async fn start_hbbs_sync_async() {
    ...
    let mut last_username = "".to_owned(); 
    ...
    loop {
        tokio::select! {
            _ = interval.tick() => {
                ...
                let current_username = crate::common::LOGIN_USERNAME.lock().unwrap().clone();
                let username_changed = current_username != last_username;
                
                if info_uploaded.uploaded && (url != info_uploaded.url || id != info_uploaded.id || username_changed) {
                    info_uploaded.uploaded = false;
                    *PRO.lock().unwrap() = false;
                    if username_changed {
                        config::Status::set("sysinfo_hash", "".to_owned());
                        last_username = current_username.clone();
                    }
                }
                ...
                if need_upload {
                    ...
                    if username_changed {
                        let hostname = if current_username.is_empty() {
                            ...
                            {
                                crate::common::DEVICE_NAME.lock().unwrap().clone()
                            }
                        } else {
...
fn heartbeat_url() -> String {
    ...
    if url.is_empty() || url.contains("rustdesk.com") || url.contains("daxian.") {
        return "".to_owned();
    }
